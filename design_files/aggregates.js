let get_terms = () => {
    ...
    terms = ['Fall 2021', 'Spring 2021', 'Fall 2022', 'Spring 2022', 'Fall 2023', 'Spring 2023'];

    const display_list = document.getElementById('terms');
    new_html = "";
    terms.forEach(element => {
        new_html += `<li class="term_element"><input class="terms_radio" type="radio" name="radAnswer" onclick="update_table()"/>${element}</li>`;
    });
...
let update_table = () => {
    ...
    results.forEach(subject => {
        ...
        <td><button class='edit_btn' onclick='edit_record(this)' id=${subject.code}>EDIT</button></td>
        </tr>
...
let edit_record = (btn) => {
    ...
    btn.setAttribute('class', 'save_btn');
    ...
    oht.innerHTML = `<input type='number' id='oht1' value=${oht.textContent} style="width: 3vw;" \>`
    project.innerHTML = `<input type='number' id='project1' value=${project.textContent} style="width: 3vw;" \>`
    lab.innerHTML = `<input type='number' id='lab1' value=${lab.textContent} style="width: 3vw;" \>`
    final.innerHTML = `<input type='number' id='final1' value=${final.textContent} style="width: 3vw;" \>`
    assign.innerHTML = `<input type='number' id='assign1' value=${assign.textContent} style="width: 3vw;" \>`
    quiz.innerHTML = `<input type='number' id='quiz1' value=${quiz.textContent} style="width: 3vw;" \>`
